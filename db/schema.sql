...
-- が存在しないため、ここでは元のままにしておくこと(9.4節側で作り直す。11節でも段階などの列を足して作り直す)。
...
-- ============================================================================
-- 11. 危機検知の作り直し 第2段階: 段階ごとの応答(2026年9月・仮の文面。心理士の確認待ち)
--
--  設定 CRISIS_RESPONSE=staged のときだけ route.ts が書き込む列(無効のときは触らない)。
--  仕組みと文面は src/crisis-response.mjs、方針は CLAUDE.md 5.16。
--  有効にする前に、この節を Supabase の SQL エディタで実行すること
--  (未実行のまま有効にしても会話は止まらず、第1段階の動きになる)。
-- ============================================================================
...
create view pending_safety as
select e.seq, e.created_at, e.risk, e.model_reason, e.session_id, s.client_id, e.subject,
       e.stage, e.decided_by, e.watch_event, e.retraction, e.teacher_answer, e.crisis_step
from safety_events e
left join sessions s on s.id = e.session_id
where e.risk <> 'none' and e.handled = false
order by e.created_at desc;
