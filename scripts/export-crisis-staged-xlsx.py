...
def main():
    ...
    put(r, ["v2 で段階を決めた規則(のべ回数)", "Tier A の発話", "それ以外"], bold)
    header(ws, r, ["v2 で段階を決めた規則(のべ回数)", "Tier A の発話", "それ以外"]); r += 1
    for lab, name in [("受動パターン", "パターンで決定"), ("キーワード", "キーワードで決定"), ("分類器", "分類器で決定"), ("慣用表現(段階1)", "慣用表現で段階1")]:
