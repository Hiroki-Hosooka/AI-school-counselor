...
- 認証は `ADMIN_TOKEN` との照合。当初はURLクエリトークン方式だったが、2026年9月に
  ログインフォーム方式に変更(合言葉はURLに乗せず `sessionStorage` に保存。README/CLAUDE.md 5.9参照)
- 評価保存は生徒側と同じ既存の `rate` アクションをそのまま利用(変更なし)
- 2026年9月(構造化面接AI統合): インテーク情報(主訴カテゴリ・つらさスケール・判定モード等)を
  表示する欄と、「ログを出力」ボタン(会話全体をMarkdown表に整形してクリップボードにコピー。
  失敗時はファイルダウンロードにフォールバック)を追加。00_統合版_構造化面接AIプロンプト.txt
  9章の仕様を参考に、心理士向けのこの画面でのみ内部判断(relation/weight/mode等)を含める
  (生徒向けの技法非開示の原則はこの画面には適用されない。CLAUDE.md 5.9)
- 2026年9月: 一覧に「低優先度セッションを隠す」トグルを追加(既定でオン)。やり取り0件、
  または3往復以下でrelation/weightが初期値(visitor/rapport)のまま=進展が無く、
  かつ危機でもないセッションを一覧から隠す。数はチェックボックスの隣に表示し、
  チェックを入れればいつでも全件見られる(消えるのではなく隠すだけ)
